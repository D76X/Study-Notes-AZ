...
6. **Azure AD** responds to the native application **over SSL** and in the response includes a **JWT Authentication Token with claims**. The response is on **SSL** to prevent the token to be tapped in. In rality though things are a bit more complicated at this point as **Azure AD** does not really send the token back to the client in the response instead **Azure AD** sends teh token back to a **predefined URL**.
(Not completely clear how it work!)

7. The native app can now make the calls to the WebApis oven **SSL** and in the heder it includes the **JWT Authentication Token** for the WebApi.

8. The WebApi reads the request and the **JWT Authentication Token it verifies it against Azure AD** and if all is OK it satifies the request by either retuning a resorce or executing a verb.
