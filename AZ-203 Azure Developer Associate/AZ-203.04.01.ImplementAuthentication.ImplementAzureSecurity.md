...
This is the simple case of a user that from their web brower visits a page of a web site which requires the user to be authenticated in order to access the resource. Fro example, in the ASP.Net realm this would be the EP of a controller on which the [Authorize] attribute is applied. When **Azure AD** is used as **authentication provider** the
**web site** must be registered with Azure AD and must hold in its configuration the **authentication URL to whichthe user browser must be redirected to in order to authenticate**.
