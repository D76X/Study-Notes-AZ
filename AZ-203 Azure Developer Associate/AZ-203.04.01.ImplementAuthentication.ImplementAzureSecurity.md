...
4. At this point once CORS is enabled the native application could make at least a GET calls to the WebApi and be able to read the responses. However, because the WebApi require authorization **before** the calls to teh WebApis are made the native application **must** go through authorization and **obtain the tokens authantication tokens** that will be later sent to teh WebApis as proof of authentication. Therefore the native application **first** go to the **Azure AD End Point** and presents its **APP_ID** os that **Azure ID DIrectory** can produce the authentication tokes it needs. 

It is **paramount** that you see the **differnce between this workflow and the previous one** as in this one there is no HTTP 302, no challenge. Clearly the native app must hold in its configuration at least the **Azure AD APP_IP, the URL of the WebAppis to call, the URL of the Azure AD End Point**.

5. The WebAppis that the native application calls are also registered with **Azure ID** and for each WebApi **Azure AD holds a list of allowed APP_IDs** for which authentication tokes will be granted. 

6. **Azure AD** responds to the native application with a **HTTP 302 redirect over SSL**. The redirect URL includes in its query parameters the **JWT Authentication Token with claims aka ID Token and the redirection URL**. The response is on **SSL** to prevent the token to be tapped in nad stolen which would make it possible for attackers to target the WebApi. The other query parameter is the **redirection URL** which is one of a set of URLs that the owner of the WebApi has provided to **Azure AD** when the WebApi is registered with the directory service.

7. The native app can now make the calls to the WebApi at the provided URL normally over **SSL** and in the header of the calls it includes the **JWT Authentication Token ID** for the WebApi so tha this can authenticate the caller.

8. The WebApi reads the request and the **JWT Authentication Token ID it verifies it against Azure AD** and if all is OK it satifies the request by either retuning a resource or executing the request with the corresponding HTTP verb.
