...
4. Specify the `Next Hop Address` so that it matches the destination address of the `Hub VNet`.
5. Oncethe `Route Table` is completed with all the routes that are required to force the traffic as desired `add the route table to any subnets you which to apply it to.`
