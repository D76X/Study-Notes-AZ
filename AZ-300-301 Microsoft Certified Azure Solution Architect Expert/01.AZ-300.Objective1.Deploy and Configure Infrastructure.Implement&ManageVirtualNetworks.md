...
In Azure it is possible to connect any two VNETs by peering them that is by `creating a logical link between them`. The **advantage of VNet Perring** over other more sophisticated architectures is that it  **requires little to no configuration**.
